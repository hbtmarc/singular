...
export function generatePushKeyForProfessionals() {
  const database = getDatabaseInstance();
  const key = push(dbRef(database, "professionals")).key;
  return key || `tmp_${Date.now()}`;
}

export async function getProfessionalsImportStatus() {
  try {
    const database = getDatabaseInstance();
    const [enabledSnap, completedAtSnap, lastImportIdSnap] = await Promise.all([
      get(dbRef(database, "professionals/__meta/importOneTimeEnabled")),
      get(dbRef(database, "professionals/__meta/importOneTimeCompletedAt")),
      get(dbRef(database, "professionals/__meta/lastImportId"))
    ]);

    const enabled = enabledSnap.exists() ? enabledSnap.val() !== false : true;
    const completedAt = completedAtSnap.exists() ? Number(completedAtSnap.val()) : null;
    const lastImportId = lastImportIdSnap.exists() ? String(lastImportIdSnap.val() || "") : "";
    ...
    return {
      ...
      completedAt: null,
      lastImportId: "",
      message: "Não foi possível consultar o status de importação de profissionais."
    };
  }
}

export async function clearProfessionalsBeforeImport(knownProfessionalIds = []) {
  const database = getDatabaseInstance();

  try {
    await set(dbRef(database, "professionals"), null);
    return {
      ok: true,
      strategy: "full-delete",
      message: "Base de profissionais limpa com sucesso."
    };
  } catch (fullDeleteError) {
    const fullDeleteDetails = formatFirebaseError(fullDeleteError, "Sem permissão para limpar /professionals inteiro.");

    try {
      await set(dbRef(database, "professionals/__meta"), null);

      const validIds = Array.isArray(knownProfessionalIds)
        ? knownProfessionalIds.map((id) => String(id || "").trim()).filter((id) => !!id && !id.startsWith("__"))
        : [];

      if (validIds.length > 0) {
        const deleteFanOut = {};
        validIds.forEach((id) => {
          deleteFanOut[`professionals/${id}`] = null;
        });
      ...
      return {
        ...
        message: "Base limpa parcialmente (meta + profissionais conhecidos).",
        warningCode: fullDeleteDetails.code,
        warningMessage: fullDeleteDetails.message
      };
    } catch (fallbackError) {
      const fallbackDetails = formatFirebaseError(fallbackError, "Falha na limpeza parcial da base de profissionais.");
      return {
        ok: false,
        strategy: "failed",
        message: "Não foi possível limpar a base de profissionais antes de importar.",
...
export function getLastProfessionalsImportId() {
  return lastProfessionalsImportId;
}

function readProfessionalCsvField(row, keys) {
...
function toBooleanAtivo(value) {
  const raw = String(value || "").trim().toLowerCase();
  if (!raw) {
    return true;
  }

  if (["false", "0", "nao", "não", "inativo"].includes(raw)) {
    return false;
  }
...
export async function bulkImportProfessionalsWithAudit(rows, options = {}) {
  const normalizedImportId = `proimp_${Date.now()}`;
  const list = Array.isArray(rows) ? rows : [];
  const onProgress = typeof options.onProgress === "function" ? options.onProgress : () => {};
  const clearBeforeImport = options?.clearBeforeImport === true;
  ...
  try {
    ...
    if (status.ok && status.enabled === false) {
      return {
        ok: false,
        message: "Importador de profissionais bloqueado (uso único já concluído).",
        stats,
        lastImportId: status.lastImportId || "",
        auditWriteOk: true
      };
    }
    ...
    if (raw && typeof raw === "object") {
      Object.keys(raw).forEach((professionalId) => {
        ...
        existing[professionalId] = raw[professionalId];
      });
    }

    if (clearBeforeImport) {
      const clearResult = await clearProfessionalsBeforeImport(Object.keys(existing));
      if (!clearResult.ok) {
        return {
          ok: false,
          message: clearResult.message || "Não foi possível limpar a base antes de importar.",
          stats,
          lastImportId: normalizedImportId,
          auditWriteOk: false,
          errorCode: clearResult.errorCode,
          errorMessage: clearResult.errorMessage
        };
      }
    }

    const staged = clearBeforeImport ? {} : { ...existing };
    const professionalsFanOut = {};
    ...
    for (let index = 0; index < list.length; index += 1) {
      ...
      try {
        const isEmptyRow = Object.values(row).every((value) => isEmptyValue(value));

        if (isEmptyRow) {
          stats.skippedEmptyRows += 1;
          auditRowsFanOut[`professionals/__meta/imports/${normalizedImportId}/rows/${rowIndex}`] = {
            rowIndex,
            status: "ignorado",
        ...
        const nomeCompleto = String(readProfessionalCsvField(row, ["nomecompleto", "nome_completo", "nome"]) || "").trim();
        const cpf = normalizeDigits(readProfessionalCsvField(row, ["cpf"]));
        const especialidade = String(readProfessionalCsvField(row, ["especialidade"]) || "").trim();
        const numeroConselho = String(readProfessionalCsvField(row, ["numeroconselho", "numero_conselho", "conselho"]) || "").trim();
        const telefone = String(readProfessionalCsvField(row, ["telefone", "telefoneprincipal"]) || "").trim();
        const email = String(readProfessionalCsvField(row, ["email"]) || "").trim().toLowerCase();
        const repasseTipo = String(readProfessionalCsvField(row, ["repassetipo", "repasse_tipo"]) || "Fixo").trim() || "Fixo";
        const repasseValor = toNumberOrZero(readProfessionalCsvField(row, ["repassevalor", "repasse_valor"]));
        const cnpjVinculado = normalizeDigits(readProfessionalCsvField(row, ["cnpjvinculado", "cnpj_vinculado"]));
        const contratoTipo = String(readProfessionalCsvField(row, ["contratotipo", "contrato_tipo"]) || "").trim();
        const pagamentoTipo = String(readProfessionalCsvField(row, ["repassepagamentotipo", "repasse_pagamento_tipo"]) || "PIX").trim() || "PIX";
        const pixChave = String(readProfessionalCsvField(row, ["repassepagamentopixchave", "repasse_pagamento_pix_chave"]) || "").trim();
        const bancoNome = String(readProfessionalCsvField(row, ["repassepagamentobanconome", "repasse_pagamento_banco_nome"]) || "").trim();
        const agencia = String(readProfessionalCsvField(row, ["repassepagamentoagencia", "repasse_pagamento_agencia"]) || "").trim();
        const conta = String(readProfessionalCsvField(row, ["repassepagamentoconta", "repasse_pagamento_conta"]) || "").trim();
        const statusPagamento = String(readProfessionalCsvField(row, ["repassepagamentostatus", "repasse_pagamento_status"]) || "").trim();
        const ativo = toBooleanAtivo(readProfessionalCsvField(row, ["ativo"]));

        if (!nomeCompleto) {
          alertas.push("Campo nomeCompleto ausente.");
        }
        if (!telefone) {
          alertas.push("Campo telefone ausente.");
        }
        if (!email) {
          alertas.push("Campo email ausente.");
        }
        if (!cpf) {
          alertas.push("CPF ausente ou inválido.");
        }

        const incomingPayload = {
          nomeCompleto: nomeCompleto || `Sem nome (linha ${rowIndex})`,
          cpf,
          especialidade,
          numeroConselho,
          telefone,
          telefoneDigits: normalizeDigits(telefone),
          email,
          repasseTipo,
          repasseValor,
          cnpjVinculado,
        ...
        const incoming = normalizeProfessionalPayload(incomingPayload);
        if (!incoming) {
        ...
        let professionalId = "";
        if (incoming.cpf.length === 11) {
          professionalId = `cpf_${incoming.cpf}`;

          if (seenCpf.has(professionalId)) {
            auditRowsFanOut[`professionals/__meta/imports/${normalizedImportId}/rows/${rowIndex}`] = {
              rowIndex,
              status: "ignorado",
          ...
          seenCpf.add(professionalId);
        } else {
          stats.semCpfCount += 1;
          professionalId = `pro_${generatePushKeyForProfessionals()}`;
        }

        const merged = normalizeProfessionalPayload(
          {
            ...(staged[professionalId] || {}),
            ...incoming
          },
          staged[professionalId] || null
        );

        staged[professionalId] = merged;
        professionalsFanOut[`professionals/${professionalId}`] = merged;
        ...
        const rowDetails = formatFirebaseError(rowError, "Erro ao processar linha da importação de profissionais.");
    ...
    if (Object.keys(professionalsFanOut).length > 0) {
      try {
        await update(dbRef(database), professionalsFanOut);
      } catch (writeError) {
        const writeDetails = formatFirebaseError(writeError, "Falha ao salvar profissionais no RTDB.");
        return {
          ok: false,
          message: "Não foi possível importar profissionais.",
    ...
    lastProfessionalsImportId = normalizedImportId;

    const metaFanOut = {
      [`professionals/__meta/importOneTimeEnabled`]: false,
      [`professionals/__meta/importOneTimeCompletedAt`]: Date.now(),
      [`professionals/__meta/lastImportId`]: normalizedImportId,
      [`professionals/__meta/imports/${normalizedImportId}/stats`]: {
        ...stats,
    ...
    try {
      await update(dbRef(database), metaFanOut);
      return {
        ok: true,
        message: "Importação de profissionais concluída com auditoria.",
        stats,
        lastImportId: normalizedImportId,
        auditWriteOk: true
      };
    } catch (auditError) {
      const auditDetails = formatFirebaseError(auditError, "Profissionais importados, mas falhou a gravação da auditoria.");
      return {
        ok: true,
        message: "Profissionais importados, mas não foi possível salvar a auditoria completa.",
        stats,
        lastImportId: normalizedImportId,
        auditWriteOk: false,
    ...
    const details = formatFirebaseError(error, "Não foi possível concluir a importação de profissionais.");
    return {
      ok: false,
      stats,
      message: "Não foi possível concluir a importação de profissionais.",
      errorCode: details.code,
      errorMessage: details.message,
      lastImportId: normalizedImportId,
      auditWriteOk: false
...
export async function readProfessionalsImportReport(importId) {
  ...
  try {
    ...
    const [statsSnap, rowsSnap] = await Promise.all([
      get(dbRef(database, `professionals/__meta/imports/${normalizedImportId}/stats`)),
      get(dbRef(database, `professionals/__meta/imports/${normalizedImportId}/rows`))
    ]);
    ...
    const details = formatFirebaseError(error, "Não foi possível carregar o relatório do import de profissionais.");
    return {
      ok: false,
      stats: null,
      rows: {},
      message: "Não foi possível carregar o relatório do import de profissionais.",
      errorCode: details.code,
      errorMessage: details.message
    };
  }
